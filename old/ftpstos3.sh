...
# Description: to transfer the files from IPA to S3 that are listed on interface_file.txt;
#              append the file names that have been succesfully transferred to file_transferred.txt;
#              sleep and poll for 60 seconds before checking for new files in the IPA to process.
...
delete_files() {
    ...
    if curl -k --user "$USERNAME:$PASSWORD" --ftp-ssl -Q "OPTS UTF8 ON" -Q "DELE $FTPS_DESTINATION_DIRECTORY/$file" "ftp://$FTPS_SERVER/"; then
        ...
        rm -f "$TEMP_DIR/$file"
        put_logs "$?" "[INFO]" "File $file deleted from temporary directory."
        return 0
    else
...
poll_files() {
    ...
    while true; do
        ...
        while IFS= read -r file; do
            if grep -qF "$file" "$TEMP_DIR/file_transferred.txt"; then
                continue
            fi
...
# Mark H. 4/23 comment : provide exit
# exit 0
